...
# A reused volume skips the initdb scripts; apply 001->010 ourselves (003 through 010 are idempotent).
if [[ "$("${PSQL[@]}" -c "SELECT to_regclass('public.users') IS NOT NULL")" != "t" ]]; then
  log "Empty database — applying migrations 001 -> 010"
  for m in 001_schema.sql 002_reference.sql 003_phase2.sql 004_phase3.sql 005_saturday.sql 006_settings.sql 007_revoked_tokens_purge.sql 008_national_id_auth_and_student_role.sql 009_whatsapp_auth_otp.sql 010_notifications_v2.sql; do
    "${PSQL[@]}" -f "/docker-entrypoint-initdb.d/$m" >/dev/null
  done
else
  log "Applying incremental idempotent migrations (003 -> 010)"
  for m in 003_phase2.sql 004_phase3.sql 005_saturday.sql 006_settings.sql 007_revoked_tokens_purge.sql 008_national_id_auth_and_student_role.sql 009_whatsapp_auth_otp.sql 010_notifications_v2.sql; do
