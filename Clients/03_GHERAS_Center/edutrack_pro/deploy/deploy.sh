...
for f in db/postgres/001_schema.sql db/postgres/002_reference.sql db/postgres/003_phase2.sql db/postgres/004_phase3.sql db/postgres/005_saturday.sql db/postgres/006_settings.sql db/postgres/007_revoked_tokens_purge.sql db/postgres/008_national_id_auth_and_student_role.sql \
...
# A reused volume skips the initdb scripts; apply 001->008 ourselves (003 through 008 are idempotent).
if [[ "$("${PSQL[@]}" -c "SELECT to_regclass('public.users') IS NOT NULL")" != "t" ]]; then
  log "Empty database — applying migrations 001 -> 008"
  for m in 001_schema.sql 002_reference.sql 003_phase2.sql 004_phase3.sql 005_saturday.sql 006_settings.sql 007_revoked_tokens_purge.sql 008_national_id_auth_and_student_role.sql; do
    "${PSQL[@]}" -f "/docker-entrypoint-initdb.d/$m" >/dev/null
  done
else
  log "Applying incremental idempotent migrations (003 -> 008)"
  for m in 003_phase2.sql 004_phase3.sql 005_saturday.sql 006_settings.sql 007_revoked_tokens_purge.sql 008_national_id_auth_and_student_role.sql; do
